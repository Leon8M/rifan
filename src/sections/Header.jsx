...
const Header = () => {
  ...
  return (
    <header className={`fixed w-full top-0 z-50 transition-colors duration-300 ${scroll ? 'bg-primary' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <a href="#" className="text-2xl font-heading font-bold text-white">RIFAN</a>
          ...
          <nav className="hidden md:flex">
            <ul className="flex items-center space-x-8">
              {navLinks.map((link) => (
                <li key={link.to}>
                  <a href={`#${link.to}`} className="text-white hover:text-secondary transition-colors duration-300 font-sub">
                    {link.title}
                  </a>
                </li>
              ))}
          ...
          {/* Mobile Menu Icon */}
          <div onClick={handleNav} className="md:hidden z-10 text-white cursor-pointer">
